# AETHER — Documentation Complète du Projet V4
## Jeu Unity 2021.3.45f1 (URP) — Bullet Heaven Roguelite
...
## ✅ Vérification de structure — CONFIRMÉE (session du 30 juin 2026)
`MetaProgressionManager.IsNodeUnlockable()` a été vérifié et confirmé conforme à la structure en losange 2+2+1 décrite ci-dessus, sans exception. Les 3 branches ont été reconstruites visuellement à partir d'une description partielle (un seul côté donné par branche) puis le côté manquant déduit et confirmé — le nœud du sommet de chaque branche nécessite bien un niveau ≥1 dans LES DEUX nœuds de la ligne du milieu. Les `nodeId`, les bonus appliqués (`GetBonusDamage`, `GetBonusArmor`, `HasSecondWind`, etc.) et les valeurs de gold en Inspector ont également été confirmés par l'utilisateur. Plus aucune action requise sur ce point.
...
## UI de l'Arbre — Réalisation visuelle (TERMINÉE et POLIE, niveau jugé professionnel)

**Structure technique** :
- `SkillTreeUI.cs` : gère le panel de détail (apparition fluide animée près du nœud cliqué, position calculée correctement via `RectTransformUtility` pour rester cohérente quel que soit le nœud, clamp aux bords d'écran 1920x1080, fermeture par clic en dehors via `ClickOutsideCatcher`), le calcul des coûts/niveaux affichés, l'appel à `MetaProgressionManager.TryBuyNode()`
- `SkillNode.cs` : géré sur chaque médaillon, gère l'état visuel (couleur d'origine + alpha si débloqué/disponible, grisé + alpha réduit si verrouillé — **jamais l'inverse**, et le manque d'or n'affecte plus l'apparence, seulement le bouton Acheter), les dots de niveau colorés par branche avec fond `DotsBackground`, nœud unique avec dot en forme de pilule (même taille que les 3 dots ensemble, plus de repositionnement dynamique nécessaire)

**Réalisation visuelle finale** : fond représentant un **grand arbre Ghibli central** qui structure naturellement la composition. Médaillons avec aura colorée distincte par branche (orange/vert/cyan). Titres de branches (Guerrier/Gardien/Fantôme) avec Outline + Underlay coloré + traits décoratifs façon bannière, pour mieux ressortir du fond. LevelDots colorés par branche avec petit fond "plaquette" discret, qui détache les indicateurs de niveau du décor sans les rendre criards.

**Bugs de polish corrigés cette session** : carré coloré parasite en Play mode (Transition Button), alpha qui se réinitialisait au lancement (vieille Image désactivée), dots qui viraient au doré au niveau max (SetDot limité à l'alpha), point unique mal centré (repensé en forme pilule plutôt que recentré par code), couleur figée en gris après déblocage (couleur d'origine capturée une seule fois dans `Awake`), nœud qui s'assombrissait à tort par manque d'or, panel de détail mal positionné/non fluide/sans fermeture au clic extérieur, warning de police TextMeshPro (✓ remplacé par ●), texte d'état des nœuds uniques recentré dans le panel de détail (`Level2Text` au lieu de `Level1Text`).
...
**Pistes de polish identifiées mais reportées (non bloquantes)** :
- Calque d'harmonisation finale sur les 15 icônes (désaturation -10%, courbes, grain Photopea) — effet jugé trop subtil pour l'effort actuel, à refaire en phase de polish final
- 3 icônes au rendu "glossy" plus inégal que le reste (Fragmentation, Nova Étendue) — tentative de régénération jugée pire que l'original, conservées telles quelles (Maîtrise du Cristal, elle, a été régénérée avec succès)
- Mise en avant visuelle des 3 nœuds ultimes (Surpuissance, Bouclier de Mana, Dash Fantôme) — jugée non nécessaire par l'utilisateur, leur position en haut au point de convergence des branches suffit déjà comme signal
...
**Décision actée et IMPLÉMENTÉE cette session** : le tir dirigé à la souris façon twin-stick shooter est maintenant disponible comme **option activable dans les Settings** ("Visée automatique" — toggle ON par défaut). Quand désactivé, le joueur vise et tire manuellement à la souris (code ajouté par l'utilisateur en autonomie). Le tir automatique reste le comportement par défaut.
...
## Réalisé
- **Fond du Menu Principal régénéré** : vue extérieure de la tour-sanctuaire au coucher de soleil, logo AETHER avec cristal bleu suspendu, bouton PLAY, cime de l'arbre des 3 branches (orange/vert/cyan) visible au loin par-dessus le mur d'enceinte — confirme visuellement que l'Arbre de compétences se trouve dans le même lieu
- TabBar avec 3 onglets : UPGRADES (= Arbre de compétences) / MENU / SETTINGS, avec **onglet actif visuellement mis en valeur** (scale + couleur, géré centralement dans `MainMenuManager.ShowPanel()`)
- **Arbre de compétences entièrement fonctionnel, visuellement abouti et polish terminé** (voir section 6)
- **SettingsPanel entièrement construit** :
  - Fond dédié : salle d'étude intérieure en pierre, palette grise/bleue sobre, cohérente avec la DA mais visuellement plus calme que les 2 autres écrans
  - Carte centrale (`SettingsCard`) avec 3 sections : AUDIO (Musique / Effets sonores, sliders branchés sur un `AudioMixer` exposé), AFFICHAGE (Plein écran / Ombres), GAMEPLAY (Visée automatique)
  - Toggles reconstruits en style "pilule" custom (glissement animé + changement de couleur via script, Transition du composant Toggle désactivée)
  - Toutes les préférences persistées via `PlayerPrefs`, propagées à la scène `Game` via `GameSceneSettingsApplier` (posé sur le `Directional Light` de cette scène) pour les Ombres

## Identité visuelle inter-écrans — décision actée cette session
Les 3 fonds (Menu / Upgrades / Settings) ne sont pas traités comme 3 wallpapers indépendants, mais comme **différentes pièces d'un même lieu** (la tour-sanctuaire), avec le **cristal bleu comme motif magique récurrent** présent dans les 3 décors. Possible signature visuelle du jeu si le motif est répété intentionnellement dans les futurs assets (icônes, effets de loot, etc.).

## À faire
- **Cadre signature** : motif de coin en pierre sculptée + éclat de cristal bleu, à générer une fois et dupliquer ×4 par rotation, pour border `SettingsCard` (et potentiellement les autres écrans) — prompt déjà préparé, pas encore exécuté
- **Calque d'harmonisation finale** des 15 icônes de compétences (désaturation/courbes/grain) — reporté volontairement à la phase de polish final du jeu
- **Nouvel onglet Personnages** : sélection entre Aether/Kael/Lyra une fois codés
- Décider si le fond du Menu Principal doit être régénéré pour un arbre plus imposant en arrière-plan, ou laissé tel quel (jugé "un peu vide" par l'utilisateur mais non bloquant)
- **Journal de quêtes évoqué mais NON VALIDÉ** comme feature — risque d'éloignement du concept bullet heaven pur à bien peser avant de s'engager dessus ; les features de longévité réellement actées restent Personnages jouables et Défis aléatoires par run (voir section 8)

---

# 11. TODO LIST — ORDRE DE PRIORITÉ MIS À JOUR

## ✅ Vérifications immédiates — TOUTES CONFIRMÉES (30 juin 2026)
- [x] `MetaProgressionManager.IsNodeUnlockable()` reflète la structure finale en losange 2+2+1
- [x] Tous les bonus de l'arbre sont bien lus et appliqués dans `HealthSystem` / `PlayerController` / `WeaponBase` / `CrystalSystem`
- [x] Valeurs finales de gold par ennemi/boss confirmées dans l'Inspector

## Priorité haute — confort et longévité
- [x] Settings complet (volume, plein écran, tir auto, ombres ON/OFF) — **TERMINÉ**
- [ ] Personnages jouables (Aether/Kael/Lyra) + onglet de sélection dans le menu
- [ ] 3 défis aléatoires par run avec récompense gold

## Priorité polish (non bloquant)
- [ ] Cadre signature (motif de coin pierre + cristal) sur SettingsCard et potentiellement les autres écrans
- [ ] Calque d'harmonisation finale sur les 15 icônes de compétences (reporté en phase de polish final)
- [ ] Décider du sort du fond du Menu Principal (régénérer avec arbre plus imposant ou garder tel quel)
...
## En réflexion — NON validé
- [ ] Journal de quêtes consultable dans le Menu — idée évoquée mais pas actée, à valider consciemment (risque d'éloignement du concept bullet heaven)
...
- **Toujours préciser fichier + classe + méthode/emplacement exact pour chaque bout de code donné** (règle explicitée cette session)
...
- **Critique visuelle toujours sincère et précise** — comparer objectivement aux meilleurs du genre (Hades, Path of Exile, Hollow Knight), identifier la cause concrète d'un défaut plutôt qu'une impression vague
- **Avant de retoucher un asset qui plaît déjà à l'utilisateur**, bien évaluer le gain réel obtenu vs l'effort demandé — ne pas insister si le gain est marginal et que l'utilisateur n'est pas convaincu
- **Ne jamais valider une feature de game design comme actée tant qu'elle n'a pas été explicitement discutée et tranchée** (ex. journal de quêtes : évoqué mais pas validé, à bien distinguer des features réellement actées)

---

*Document mis à jour — Projet Aether — 30 juin 2026*
*Cette version (V4) remplace AETHER_PROJECT_COMPLET_V3.md*
*Accompagné de aether_conversation_save.json (mis à jour le 30 juin 2026) pour le détail exhaustif de cette session*
